...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 19 15:38:08 2023
...
operaciones = pd.DataFrame(columns=['Tipo','Saldo', 'Cantidad', 'Precio','Tamaño $'])
...
for i in range(len(df)):
    ...
    if df['Signal'].iloc[i] == 1 and posicion_crypto == 0:
        ...
        aux = pd.DataFrame({'Tipo':['Compra'],'Saldo':[saldo_usd], 'Cantidad':[posicion_crypto], 'Precio':[precio_crypto],'Tamaño $':[posicion_crypto*precio_crypto]})
        operaciones = pd.concat([operaciones,aux],ignore_index=True)
    ...
    # Si la señal es de venta y tenemos una posición en criptomoneda
        ...
        saldo_usd += posicion_crypto * precio_crypto
        aux = pd.DataFrame({'Tipo':['Venta'],'Saldo':[saldo_usd], 'Cantidad':[posicion_crypto], 'Precio':[precio_crypto],'Tamaño $':[posicion_crypto*precio_crypto]})
        operaciones = pd.concat([operaciones,aux],ignore_index=True)
        posicion_crypto = 0
...
# Exportar las operaciones a un archivo CSV
operaciones.to_csv('operaciones.csv', index=False)